...
def enter_state_b():
    global new_state, old_state
    if old_state != STATE_A and new_state != old_state or old_state == None:
        GPIO.output(S1, GPIO.HIGH)
        GPIO.output(K1, GPIO.HIGH)
        print('state F')
        print('enter_b_error')
    ...
    new_state = STATE_B
    time.sleep(2)
    print('b old',old_state,'b new',new_state)
    # 狀態 B 下需要執行的代碼
...
def enter_state_d():
    ...
    '''
    if old_state != STATE_B and new_state != old_state :
    ...
    if new_state == STATE_B or new_state == old_state or old_state ==STATE_B:
        ...
        if GPIO.input(S1) == GPIO.LOW:
            ...
            print('state F')
            print('k1 cannot close before s1 close')
    else :
        print('else')

    print('d old', old_state, 'd new', new_state)
